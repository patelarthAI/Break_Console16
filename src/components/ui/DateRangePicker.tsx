        ...
        {cells.map((cell, i) => {
          ...
          return (
            <button
              ...
              className={`
                h-8 rounded-lg text-[12px] font-semibold transition-all relative
                ${isFuture ? 'text-slate-700 cursor-not-allowed' : 'cursor-pointer'}
                ${isWeekend && !inRange ? 'text-slate-600' : ''}
                ${isStart || isEnd
                  ? 'bg-indigo-500 text-white font-bold shadow-[0_0_12px_rgba(99,102,241,0.5)] z-10'
                  : inRange
                    ? 'bg-indigo-500/15 text-indigo-300'
                    : isToday
                      ? 'bg-white/5 text-white ring-1 ring-indigo-500/40'
  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <button
        ...
        className={`group flex items-center gap-3 w-full px-4 py-3 rounded-xl bg-white/[0.03] border transition-all duration-300 backdrop-blur-xl ${
          isOpen
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-[100] left-0 mt-2 rounded-2xl bg-[#0a001a]/95 border border-white/10 backdrop-blur-3xl shadow-[0_24px_64px_rgba(0,0,0,0.9)] overflow-hidden"
            style={{ width: 'max(100%, 620px)' }}
          >
            <div className="flex">
              {/* Left: Presets */}
              <div className="w-[180px] border-r border-white/5 p-3 space-y-0.5 flex-shrink-0">
                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest px-3 pt-1 pb-2">Quick Select</p>
                {PRESETS.map(p => {
