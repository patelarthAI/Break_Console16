...
export async function GET(request: NextRequest) {
    // Auth — fail closed. This endpoint mutates data (auto-logouts, auto-leaves),
    // so it must never run without a valid secret. Accept the secret via ?key=
    // (external cron services) or an Authorization: Bearer header (Vercel Cron).
    const cronSecret = process.env.CRON_SECRET;
    if (!cronSecret) {
        return Response.json({ error: 'Cron not configured (CRON_SECRET missing)' }, { status: 503 });
    }
    const providedKey = request.nextUrl.searchParams.get('key');
    const authHeader = request.headers.get('authorization') ?? '';
    const bearer = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : '';

    if (providedKey !== cronSecret && bearer !== cronSecret) {
    ...
    try {
        ...
        const { data: leavesData, error: leavesError } = await supabaseServer
            .from('leaves')
            .select('id, date, client_name, employee_name, is_planned, leave_type, day_count, created_at, approver, reason')
            .in('date', datesToProcess)
