...
// ─── Core Utilities ──────────────────────────────────────────────────────────
export async function getLatestDateWithData(): Promise<string> {
    ...
    if (error || !data) return getTodayKey();
    return data.date;
}
...
const STATS_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes (Reports don't need to be minute-fresh)
...
function withSyntheticAutoLogout(logs: TimeLog[], user: User, date: string): TimeLog[] {
    ...
    if (stats.status === 'offline' || stats.status === 'idle' || stats.status === 'punched_out' || stats.status === 'on_leave') {
        return logs;
    }
...
export async function getLeaves(clientName?: string, force = false): Promise<LeaveRecord[]> {
    ...
    return withCachedQuery(`leaves:${clientName ?? '*'}`, LEAVES_CACHE_TTL_MS, async () => {
        let query = supabase
            .from('leaves')
            .select('id, date, client_name, employee_name, is_planned, leave_type, day_count, created_at, reason, approver')
...
export async function getLeavesPage(options?: LeavePageOptions): Promise<PaginatedLeaveResult> {
    ...
    return withCachedQuery(cacheKey, LEAVES_CACHE_TTL_MS, async () => {
        let query = supabase
            .from('leaves')
            .select('id, date, client_name, employee_name, is_planned, leave_type, day_count, created_at, approver, reason', { count: 'exact' })
            .neq('leave_type', 'Dismissed')
            .neq('leave_type', 'System: Declined')
            .range(from, to);
        ...
        if (leaveType) {
            if (Array.isArray(leaveType)) { if (leaveType.length > 0) query = query.in('leave_type', leaveType); }
            else query = query.ilike('leave_type', leaveType);
        }
...
export async function getSmartLeaves(dates: string[], options?: { includeHistoricalLeaves?: boolean }): Promise<SmartLeaveRecord[]> {
    ...
    const { data, error } = await supabase
        .from('leaves')
        .select('id, date, client_name, employee_name, is_planned, leave_type, day_count, created_at')
        .in('date', dates)
...
// ─── Session (browser-local, persisted) ──────────────────────────────────────────
...
export async function toggleManualFlag(userId: string, date: string, adminId: string): Promise<boolean> {
    const logs = await getLogs(userId, date);
    const existing = logs.find(l => l.eventType === 'manual_flag');
    
    if (existing) {
        await deleteTimeLog(existing.id);
...
function deriveStatus(logs: TimeLog[]): Omit<UserStatusRecord, 'user'> {
    ...
    for (const log of logs) {
        if (!punchIn && log.eventType !== 'punch_in') continue;
        switch (log.eventType) {
            ...
            case 'break_start':
                ...
                if (currentWorkSegStart) { workedMs += log.timestamp - currentWorkSegStart; currentWorkSegStart = undefined; } break;
            case 'break_end':
                status = 'working'; workStart = log.timestamp;
                currentWorkSegStart = log.timestamp; 
                if (breakStart) { breakMs += log.timestamp - breakStart; breakStart = undefined; } break;
            case 'brb_start':
...
export async function getAllUsersStatus(clientName?: string, force = false): Promise<UserStatusRecord[]> {
    return withCachedQuery(`status:${clientName ?? '*'}`, 30000, async () => { // 30s cache to save Supabase egress
        const today = getTodayKey();
        const yesterday = getPastDaysZoned(1, true)[0] ?? today;
        ...
        // Egress-conscious: fetch TODAY only (the common case). Just one date partition.
        const { data: todayData, error: todayError } = await supabase
            ...
            .eq('date', today)
            .order('timestamp', { ascending: true });
        assertSupabaseOk(todayError, 'Failed to load status logs');

        const logsByUserDate = buildLogsByUserDate((todayData ?? []) as LogRow[]);

        // Cross-midnight only matters when a user has activity today but no punch_in
        // (their session began yesterday). Only then do we pay for the extra read —
        // i.e. roughly the post-midnight window, not all day.
        const needsYesterday = usersData.some((row: UserRow) => {
            const tl = getLogsForUserDate(logsByUserDate, row.id, today);
            return tl.length > 0 && !tl.some(l => l.eventType === 'punch_in');
        });

        if (needsYesterday) {
            const { data: yestData, error: yestError } = await supabase
                .from('time_logs')
                .select(LOG_SELECT)
                .in('user_id', userIds)
                .eq('date', yesterday)
                .order('timestamp', { ascending: true });
            assertSupabaseOk(yestError, 'Failed to load prior-day status logs');
            for (const row of (yestData ?? []) as LogRow[]) {
                const key = getLogBucketKey(row.user_id, row.date);
                const bucket = logsByUserDate.get(key) ?? [];
                bucket.push(rowToLog(row));
                logsByUserDate.set(key, bucket);
            }
        }

        return usersData.map((row: UserRow) => {
            const user = rowToUser(row);
            const todayLogs = getLogsForUserDate(logsByUserDate, user.id, today);
            const yestLogs = getLogsForUserDate(logsByUserDate, user.id, yesterday);
            const todayHasPunchIn = todayLogs.some(l => l.eventType === 'punch_in');

            // Cross-midnight continuation: stitch yesterday's punch_in when needed.
            let sessionLogs = todayLogs;
            let sessionDate = today;
            if (todayLogs.length > 0 && !todayHasPunchIn && yestLogs.length > 0) {
                sessionLogs = [...yestLogs, ...todayLogs].sort((a, b) => a.timestamp - b.timestamp);
                sessionDate = yesterday;
            }

            const sessionWithAutoLogout = withSyntheticAutoLogout(sessionLogs, user, sessionDate);

            return {
                user,
                ...deriveStatus(sessionWithAutoLogout),
            };
        });
    }, force);
}

export async function getSingleUserStatus(userId: string): Promise<UserStatusRecord | null> {
    const today = getTodayKey();
    ...
    const user = rowToUser(userData as UserRow);
    const todayLogs = await getLogs(userId, today);
    const todayHasPunchIn = todayLogs.some(l => l.eventType === 'punch_in');

    // Cross-midnight continuation (mirror of getAllUsersStatus). Only pay for the
    // prior-day read when today has activity but no punch_in (post-midnight window).
    let sessionLogs = todayLogs;
    let sessionDate = today;
    if (todayLogs.length > 0 && !todayHasPunchIn) {
        const yesterday = getPastDaysZoned(1, true)[0] ?? today;
        const yestLogs = await getLogs(userId, yesterday);
    ...
    const logsWithAutoLogout = withSyntheticAutoLogout(sessionLogs, user, sessionDate);

    return {
        user,
        ...deriveStatus(logsWithAutoLogout),
    };
}
...
export async function get7DayBreakStats(clientName?: string, force = false): Promise<UserBreakStats[]> {
    return withCachedQuery(`seven-day-break-stats:${clientName ?? '*'}`, STATS_CACHE_TTL_MS, async () => {
        // Cross-midnight sessions in historical stats are handled by the daily-sync
        // cron, which inserts an auto_logout on the session's start day — so we only
        // need each day's own partition here (keeps Supabase egress minimal).
...
export interface WeeklyBreakStats {
    ...
    leaveDays: number;       // total leave days in this period
...
export async function getWeeklyBreakStats(clientName?: string, force = false): Promise<WeeklyBreakStats[]> {
    return withCachedQuery(`weekly-stats:${clientName ?? '*'}`, STATS_CACHE_TTL_MS, async () => {
        ...
        const userIds = usersData.map((u: UserRow) => u.id);
        // Cross-midnight historical sessions are closed same-day by the daily-sync
        // cron, so each day's own partition is sufficient (minimal Supabase egress).
        const { data: logsData, error: logsError } = await supabase
            .from('time_logs')
            .select(LOG_SELECT)
            .in('user_id', userIds)
        ...
        const { data: leavesData, error: leavesError } = await supabase
            .from('leaves')
            .select('employee_name, client_name, date')
            .in('date', days);
        assertSupabaseOk(leavesError, 'Failed to load leaves for weekly stats');

        const leavesByUser = new Map<string, Set<string>>();
        (leavesData ?? []).forEach(l => {
            const key = `${l.employee_name.toLowerCase().trim()}|${l.client_name.toLowerCase().trim()}`;
            if (!leavesByUser.has(key)) leavesByUser.set(key, new Set());
            leavesByUser.get(key)!.add(l.date);
        });
        ...
        return usersData.map((row: UserRow) => {
            ...
            const userLeaves = leavesByUser.get(`${user.name.toLowerCase().trim()}|${user.clientName.toLowerCase().trim()}`);

            for (const day of days) {
                if (userLeaves?.has(day)) {
                    leaveDays++;
                }
