...
/** Alias for formatDuration to match elite naming conventions */
export const formatMs = formatDuration;

/** Returns formatted time and date for the Elite Dashboard Clock */
export function getEliteTime(ts: number, timezone = 'America/Chicago') {
    ...
    }).format(new Date(ts)).toUpperCase();
...
export function getElapsedWeekdays(): { days: string[]; isLastWeek: boolean } {
    ...
    // Create a UTC date at midnight to represent the Chicago day without local TZ interference
    const todayChicago = new Date(Date.UTC(todayY, todayM, todayD));

    // Find the most recent Monday
    const daysSinceMonday = dow === 0 ? 6 : dow - 1;
    const targetMonday = new Date(todayChicago);
    targetMonday.setUTCDate(todayChicago.getUTCDate() - daysSinceMonday);
    ...
    for (let i = 0; i < daysToGenerate; i++) {
        ...
        // Direct string construction to avoid TZ conversion
...
export function computeWorkedTime(session: DaySession, now: number, date?: string, shiftEnd = '17:00'): number {
    ...
    const totalBrb = computeTotalTime(session.brbs, end);
    return Math.max(0, end - session.punchIn - totalBreak - totalBrb);
}
